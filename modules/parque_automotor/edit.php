...
$page_title = 'Editar Vehículo - Parque Automotor';
$success_message = '';
$error_message = '';
$vehiculo = null;
...
$stmt = $mysqli->prepare("SELECT * FROM parque_automotor WHERE id = ?");
...
// Obtener archivos existentes del vehículo
...
$archivos_existentes = $archivos_query->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['placa']) || empty($data['marca']) || empty($data['modelo'])) {
        ...
        $check_stmt = $mysqli->prepare("SELECT id FROM parque_automotor WHERE placa = ? AND id != ?");
        $check_stmt->bind_param("si", $data['placa'], $vehiculo_id);
        $check_stmt->execute();
        
        if ($check_stmt->get_result()->num_rows > 0) {
            $error_message = 'Ya existe otro vehículo con esta placa.';
        } else {
            if (empty($error_message)) {
                // Actualizar vehículo
                $sql = "UPDATE parque_automotor SET 
                    placa = ?, marca = ?, modelo = ?, nib = ?, chassis = ?, motor = ?, 
                    radio_telefono = ?, serial = ?, compania_soat = ?, soat = ?, 
                    vencimiento_soat = ?, certificado_movilizacion = ?, fecha_vencimiento_certificado = ?,
                    tipo_combustible = ?, tarjeta_operacion = ?, fecha_tarjeta_operacion = ?,
                    inicio_tarjeta_operacion = ?, final_tarjeta_operacion = ?, revision_preventiva = ?,
                    vencimiento_preventiva = ?, poliza_responsabilidad_civil = ?, observaciones = ?,
                    empresa = ?, asociado_id = ?, estado = ?, usuario_actualizacion = ?
                WHERE id = ?";
                
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("sssssssssssssssssssssssssii",
                    ...
                    $data['empresa'], $data['asociado_id'], $data['estado'], $_SESSION['user_id'], $vehiculo_id
                );
                ...
                if ($stmt->execute()) {
                    ...
                    if (isset($_FILES['archivos']) && !empty($_FILES['archivos']['name'][0])) {
                        ...
                        if (!empty($upload_errors)) {
                            $_SESSION['message'] = 'Vehículo actualizado exitosamente, pero hubo problemas con algunos archivos: ' . implode(', ', $upload_errors);
                            $_SESSION['message_type'] = 'warning';
                        } else {
                            $_SESSION['message'] = 'Vehículo actualizado exitosamente con todos los archivos.';
                            $_SESSION['message_type'] = 'success';
                        }
                    } else {
                        $_SESSION['message'] = 'Vehículo actualizado exitosamente.';
                        $_SESSION['message_type'] = 'success';
                    }
                    ...
                    $error_message = 'Error al actualizar el vehículo: ' . $mysqli->error;
                }
            }
        }
        $check_stmt->close();
    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2><i class="fas fa-edit me-2"></i>Editar Vehículo - <?php echo htmlspecialchars($vehiculo['placa']); ?></h2>
                <div>
                    <a href="view.php?id=<?php echo $vehiculo_id; ?>" class="btn btn-info me-2">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        <i class="fas fa-edit me-2"></i>Editar Información del Vehículo
                    </h5>
                </div>
                <div class="card-body">
                    <form method="POST" class="needs-validation" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="radio_telefono" name="radio_telefono" 
                                       value="<?php echo htmlspecialchars($vehiculo['radio_telefono']); ?>" 
                                       maxlength="50">
                            </div>
                            <div class="col-md-4">
                                <label for="serial" class="form-label">Serial</label>
                                <input type="text" class="form-control" id="serial" name="serial" 
                                       value="<?php echo htmlspecialchars($vehiculo['serial']); ?>" 
                                       maxlength="50">
                            </div>
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="tipo_combustible" name="tipo_combustible">
                                    ...
                                    <option value="diesel" <?php echo ($vehiculo['tipo_combustible'] === 'diesel') ? 'selected' : ''; ?>>Diesel</option>
                                    <option value="gas" <?php echo ($vehiculo['tipo_combustible'] === 'gas') ? 'selected' : ''; ?>>Gas</option>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="date" class="form-control" id="vencimiento_soat" name="vencimiento_soat" 
                                       value="<?php echo $vehiculo['vencimiento_soat']; ?>">
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="certificado_movilizacion" name="certificado_movilizacion" 
                                       value="<?php echo htmlspecialchars($vehiculo['certificado_movilizacion']); ?>" 
                                       maxlength="50">
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" id="fecha_vencimiento_certificado" name="fecha_vencimiento_certificado" 
                                       value="<?php echo $vehiculo['fecha_vencimiento_certificado']; ?>">
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control" id="tarjeta_operacion" name="tarjeta_operacion" 
                                       value="<?php echo htmlspecialchars($vehiculo['tarjeta_operacion']); ?>" 
                                       maxlength="50">
                            </div>
                            <div class="col-md-3">
                                ...
                                <input type="date" class="form-control" id="fecha_tarjeta_operacion" name="fecha_tarjeta_operacion" 
                                       value="<?php echo $vehiculo['fecha_tarjeta_operacion']; ?>">
                            </div>
                            <div class="col-md-3">
                                ...
                                <input type="date" class="form-control" id="inicio_tarjeta_operacion" name="inicio_tarjeta_operacion" 
                                       value="<?php echo $vehiculo['inicio_tarjeta_operacion']; ?>">
                            </div>
                            <div class="col-md-3">
                                ...
                                <input type="date" class="form-control" id="final_tarjeta_operacion" name="final_tarjeta_operacion" 
                                       value="<?php echo $vehiculo['final_tarjeta_operacion']; ?>">
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="revision_preventiva" name="revision_preventiva" 
                                       value="<?php echo htmlspecialchars($vehiculo['revision_preventiva']); ?>" 
                                       maxlength="50">
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="date" class="form-control" id="vencimiento_preventiva" name="vencimiento_preventiva" 
                                       value="<?php echo $vehiculo['vencimiento_preventiva']; ?>">
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="poliza_responsabilidad_civil" name="poliza_responsabilidad_civil" 
                                       value="<?php echo htmlspecialchars($vehiculo['poliza_responsabilidad_civil']); ?>" 
                                       maxlength="50">
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="empresa" name="empresa" 
                                       value="<?php echo htmlspecialchars($vehiculo['empresa']); ?>" 
                                       maxlength="100">
                            </div>
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="asociado_id" name="asociado_id">
                                    ...
                                    <option value="<?php echo $asociado['id']; ?>" 
                                            <?php echo ($vehiculo['asociado_id'] == $asociado['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="observaciones" name="observaciones" rows="4"><?php echo htmlspecialchars($vehiculo['observaciones']); ?></textarea>
                            </div>
                        </div>

                        <!-- Archivos existentes -->
                        <?php if ($archivos_existentes->num_rows > 0): ?>
                        <div class="row mb-4">
                            <div class="col-12">
                                <h6 class="text-primary border-bottom pb-2 mb-3">
                                    <i class="fas fa-file-alt me-2"></i>Archivos Existentes
                                </h6>
                                <div class="table-responsive">
                                    <table class="table table-sm">
                                        ...
                                        <tbody>
                                            <?php while ($archivo = $archivos_existentes->fetch_assoc()): ?>
                                            <tr id="archivo_<?php echo $archivo['id']; ?>">
                                                <td>
                                                    <?php echo htmlspecialchars($archivo['nombre_original']); ?>
                                                    <?php if ($archivo['descripcion']): ?>
                                                        <br><small class="text-muted"><?php echo htmlspecialchars($archivo['descripcion']); ?></small>
                                                    <?php endif; ?>
                                                </td>
                                                <td>
                                                    <span class="badge bg-info">
                                                        <?php echo ucfirst(str_replace('_', ' ', $archivo['tipo_documento'])); ?>
                                                    </span>
                                                </td>
                                                <td><?php echo formatDate($archivo['fecha_subida']); ?></td>
                                                <td>
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <h6 class="text-primary border-bottom pb-2 mb-3">
                                    <i class="fas fa-file-upload me-2"></i>Agregar Nuevos Archivos (Opcional)
                                </h6>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="view.php?id=<?php echo $vehiculo_id; ?>" class="btn btn-info">
                                        <i class="fas fa-eye me-1"></i>Ver Detalles
                                    </a>
                                    <a href="index.php" class="btn btn-secondary">
    ...
    if (confirm('¿Está seguro que desea eliminar este archivo? Esta acción no se puede deshacer.')) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    $('#archivo_' + archivoId).fadeOut(function() {
                        $(this).remove();
                    });
