...
-- Tabla de conductores (para futura expansión)
...
-- Tabla de vehículos (para futura expansión)
CREATE TABLE IF NOT EXISTS `vehiculos` (
    ...
    `fecha_actualizacion` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    KEY `idx_placa_vehiculo` (`placa`),
    KEY `idx_asociado_vehiculo` (`asociado_id`),
    FOREIGN KEY (`asociado_id`) REFERENCES `asociados`(`id`) ON DELETE SET NULL,
...
ALTER TABLE `conductores` COMMENT = 'Información de conductores (para futura expansión)';
ALTER TABLE `vehiculos` COMMENT = 'Información de vehículos (para futura expansión)';
